    ...
    public class SyphonServer : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (_serverInstance == IntPtr.Zero)
            {
                ...
                _serverInstance = Plugin_CreateServer(Name ?? gameObject.name, width, height);

                IntPtr metalTextureId = Plugin_GetServerTexture(_serverInstance);
                // Create the server texture as an external 2D texture.
                _serverTexture = Texture2D.CreateExternalTexture(
                    width, height, TextureFormat.BGRA32, false, false,
                    metalTextureId
                );
            }
