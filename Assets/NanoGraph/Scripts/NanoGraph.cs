  ...
  public class NanoGraph {
    ...
    public void Validate() {
      ...
      while (toVisit.TryDequeue(out IDataNode node)) {
        ...
        if (!StructuralComparisons.StructuralEqualityComparer.Equals(cacheData.InputSpec, inputSpec)) {
        ...
        if (!StructuralComparisons.StructuralEqualityComparer.Equals(cacheData.OutputSpec, outputSpec)) {
          invalidatedNodes.Add(node);
    ...
    private class CodeGeneratorFromComputeNode : ICodeGenerator {
      ...
      public CodeGeneratorFromComputeNode(IComputeNode node) {
        Node = node;
      }

      public void SetInput(CodeCachedResult inputResult) {
      ...
      public void EmitCode(CodeContext context) {
        ...
        Node.EmitLoadOutputsCode(context, InputResult.Value);
      }
    }

    private List<ICodeGenerator> GenerateComputePlan(IComputeNode root, List<CodeGeneratorFromComputeNode> dependentCalcNodes, List<string> errors) {
      ...
      return computeNodeOrder.Keys.Cast<IDataNode>().Reverse().Select<IDataNode, ICodeGenerator>(node => {
        if (node is ICodeNode codeNode) {
          ...
          var codeGenerator = new CodeGeneratorFromComputeNode(computeNode);
          dependentCalcNodes.Add(codeGenerator);
    ...
    public void DoSomething() {
      ...
      var typeStandardVertexNode = AddNewNode(new TypeDeclNode { Fields = {
    ...
    public void GenerateProgram(IReadOnlyList<IComputeNode> outputNodes) {
      ...
      Dictionary<IComputeNode, CodeCachedResult> computeNodeResults = new Dictionary<IComputeNode, CodeCachedResult>();
      ...
      var computePlans = new List<(IComputeNode node, List<ICodeGenerator>, List<CodeGeneratorFromComputeNode>)>();
      while (queuedCalcNodes.TryDequeue(out IComputeNode node)) {
        ...
        List<ICodeGenerator> computePlan = GenerateComputePlan(node, dependentComputeNodes, errors);
        computePlans.Add((node, computePlan, dependentComputeNodes));
      ...
      foreach (var (computeNode, computePlan, dependentCalcNodes) in computePlans) {
        // Read inputs.
      ...
      foreach (var outputNode in outputNodes) {
        ...
        if (!computeNodeResults.TryGetValue(outputNode, out CodeCachedResult cachedResult)) {
          errors.Add($"Missing result for output node {outputNode}");
          continue;
        }
        foreach (var field in outputSpec.Fields) {
          string fieldIdentifier = cachedResult.ResultType.GetField(field.Name);
          ...
          getOutputFunc.AddStatement($"return {cachedResult.Result.Identifier}.{fieldIdentifier};");
          finalOutputIndex++;
        }
      }
    ...
    private static TypeDecl TypeDeclFromDataFields(IEnumerable<DataField> fields) {
      return new TypeDecl(fields.Where(field => !field.IsCompileTimeOnly).Select(field => new TypeField { Name = field.Name, Type = field.Type }).ToArray());
    }
