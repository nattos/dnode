...
namespace NanoGraph.VisualScripting {
  public abstract class VisualScriptingCanvas<TGraph> : ICanvas
    ...
    private void HandleViewportInput() {
      ...
      if (isMouseOver && e.controlType == EventType.ScrollWheel) {
        ...
        if (zooming) {
          ...
          var zoomDelta = -e.mouseDelta.y * BoltCore.Configuration.zoomSpeed;
          zoomDelta = Mathf.Clamp(graph.zoom + zoomDelta, GraphGUI.MinZoom, 2.0f) - graph.zoom;
    ...
    public bool isGrouping => false;//isLassoing && (e.ctrlOrCmd && graph.elements.Includes<GraphGroup>());
    ...
    public void DeleteSelection() {
      ...
      if (deleteGroup.Count == 1 && deleteGroup.First() is IUnit unit) {
        var input = unit.inputs.FirstOrDefault();
        var output = unit.outputs.FirstOrDefault();
        var inputConnection = input?.connections?.FirstOrDefault();
        var outputConnection = output?.connections?.FirstOrDefault();
        if (inputConnection != null && inputConnection.source != null && outputConnection != null && outputConnection.destination != null) {
          UndoUtility.RecordEditedObject("Delete Graph Element");
          inputConnection.source.ValidlyConnectTo(outputConnection.destination);
        }
      }
    ...
    private void HandleDeleting() {
      if (e.IsValidateCommand("Delete")) {
        ...
        DeleteSelection();
      } else if (e.IsKeyDown(KeyCode.Delete) || e.IsKeyDown(KeyCode.Backspace)) {
        DeleteSelection();
      }
    }
