...
namespace NanoGraph.VisualScripting {
  public interface ILabelSizeGroupElement {
    ILabelSizeGroupElement[] Group { get; set; }
    float PreferredWidth { get; set; }
    float GetMinLabelWidth();
  }

  [Widget(typeof(IUnitPort))]
  public abstract class UnitPortWidget<TPort> : Widget<FlowCanvas, TPort>, IUnitPortWidget, ILabelSizeGroupElement
      where TPort : class, IUnitPort {
    ...
    public override void CachePosition() {
      ...
      if (showLabel) {
        var labelPosition = new Rect(x, y, GetLabelWidth(), GetLabelHeight());
      ...
      if (showInspector) {
        const float MinInspectorWidth = 30.0f;
        float width = _preferredWidth - (x - unitPosition.x); //GetInspectorWidth(),;
        if (width < MinInspectorWidth) {
          float diff = MinInspectorWidth - width;
          x -= diff;
          width += diff;
        }
        var inspectorPosition = new Rect(x, y, width, GetInspectorHeight());
    ...
    private float GetLabelWidth() {
      const float WidthQuantization = 20.0f;
  
      var group = this as ILabelSizeGroupElement;
      if (group.Group == null) {
        return group.GetMinLabelWidth();
      }
      float maxWidth = 0.0f;
      // float thisLabelWidth = group.GetMinLabelWidth();
      bool useQuantizedWidth = false;
      foreach (ILabelSizeGroupElement element in group.Group) {
        float width = element.GetMinLabelWidth();
        if (element != group) {
          if (width > WidthQuantization) {
            useQuantizedWidth = true;
            break;
          }
        }
        maxWidth = System.Math.Max(maxWidth, width);
      }
      if (useQuantizedWidth) {
        return Mathf.Ceil(group.GetMinLabelWidth() / WidthQuantization) * WidthQuantization;
      }
      return maxWidth;
    }

    ILabelSizeGroupElement[] ILabelSizeGroupElement.Group { get; set; }
    float ILabelSizeGroupElement.GetMinLabelWidth() {
      return Mathf.Min(Styles.label.CalcSize(labelContent).x, Styles.maxLabelWidth);
    }

    float _preferredWidth = 0.0f;
    float ILabelSizeGroupElement.PreferredWidth {
      get => _preferredWidth;
    ...
    protected bool showIcon => false;//description.icon != null;
      ...
      if (colorIfActive) {
        foreach (var connection in connections) {
          ...
          if (EditorApplication.isPaused) {
            ...
            color = Color.Lerp(UnitConnectionStyles.activeColor, color, (EditorTimeBinding.time - connectionEditorData.lastInvokeTime) / UnitWidget<BaseNode>.Styles.invokeFadeDuration);
          }
        }
      }
