...
namespace NanoGraph.VisualScripting {
  ...
  public class UnitWidget<TUnit> : NodeWidget<FlowCanvas, TUnit>, IUnitWidget where TUnit : class, IUnit {
    ...
    protected virtual void CacheDefinition() {
      ...
      ILabelSizeGroupElement[] inputGroup = inputs.OfType<ILabelSizeGroupElement>().ToArray();
      ILabelSizeGroupElement[] outputGroup = outputs.OfType<ILabelSizeGroupElement>().ToArray();
      foreach (ILabelSizeGroupElement input in inputGroup) {
        input.Group = inputGroup;
      }
      foreach (ILabelSizeGroupElement output in outputGroup) {
    ...
    public override void CachePosition() {
      ...
      // var portsWidth = Styles.CardWidth;
      ...
      // var settingsWidth = Styles.CardWidth;
      // if (showSettings) {
      //   foreach (var setting in settings) {
      //     var settingWidth = 0f;
      ...
      var headerAddonWidth = Styles.CardWidth;
      // if (showHeaderAddon) {
      ...
      float titleWidth = Styles.CardWidth;
      float headerTextWidth = titleWidth;
      // float surtitleWidth = 0f;
      ...
      // var headerWidth = Styles.CardWidth; // Mathf.Max(headerTextWidth + iconsWidth, Mathf.Max(settingsWidth, headerAddonWidth));// + Styles.iconSize + Styles.spaceAfterIcon;
      var innerWidth = Styles.CardWidth; // Mathf.Max(portsWidth, headerWidth);
      ...
      var headerTextX = innerOrigin.x; // iconPosition.xMax + Styles.spaceAfterIcon;
      var y = innerY;
      var headerHeight = 0f;
      var surtitleHeight = 0f;

      if (showSurtitle) {
        surtitleHeight = Styles.surtitle.lineHeight; // Styles.surtitle.CalcHeight(surtitleContent, headerTextWidth);
      ...
      var titleHeight = 0f;
      if (showTitle) {
        titleHeight = Styles.title.lineHeight; // Styles.title.CalcHeight(titleContent, headerTextWidth);
      ...
      if (showSubtitle) {
        ...
        subtitleHeight = Styles.subtitle.lineHeight; // Styles.subtitle.CalcHeight(subtitleContent, headerTextWidth);
      ...
      if (showPorts) {
        ...
        var portsHeight = System.Math.Max(inputsHeight, outputsHeight);
        ...
        portsBackgroundPosition = new Rect(
            edgeX, portsBackgroundY,
            edgeWidth, portsBackgroundHeight);

        float maxOutputWidth = 0.0f;
        foreach (var output in outputs) {
          float outputWidth = output.GetInnerWidth();
          maxOutputWidth = System.Math.Max(maxOutputWidth, outputWidth);
        }
        float inputWidth = innerWidth - maxOutputWidth;
        foreach (ILabelSizeGroupElement input in inputs.OfType<ILabelSizeGroupElement>()) {
          input.PreferredWidth = inputWidth;
        }
        foreach (ILabelSizeGroupElement output in outputs.OfType<ILabelSizeGroupElement>()) {
          output.PreferredWidth = maxOutputWidth;
        }
      }

      var edgeHeight = InnerToEdgePosition(new Rect(0, 0, 0, innerHeight)).height;

      _position = new Rect(edgeX, edgeY, edgeWidth, edgeHeight);
    }
    ...
    protected void DrawTitle() {
      string titleContentText = titleContent.text;
      if (this.unit is NodeBasedNode nodeBasedNode) {
        titleContentText = nodeBasedNode.Node?.ShortName ?? titleContentText;
      } else if (this.unit is EmbeddedNode embeddedNode) {
        if (embeddedNode.ScriptGraphAssetReference) {
          titleContentText = embeddedNode.ScriptGraphAssetReference.name ?? titleContentText;
        }
      }
    ...
    public static class Styles {
      ...
      public const float CardWidth = 175.0f;
    }
  }
}
