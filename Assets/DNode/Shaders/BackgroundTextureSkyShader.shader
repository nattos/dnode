Shader "BackgroundTextureSkyShader"
{
    Properties
    {
        [NoScaleOffset]_Texture2D("Texture2D", 2D) = "white" {}
        _TextureAlpha("Float", Float) = 0
	}
    ...
    {
        float4 skyColor = RenderSky(input);
        skyColor.rgb *= GetCurrentExposureMultiplier();

		float2 uv = input.positionCS;
		uv.x *= (_ScreenParams.z - 1.0);
		uv.y *= (_ScreenParams.w - 1.0);
        float4 texColor = SAMPLE_TEXTURE2D(_Texture2D, sampler_Texture2D, uv);

        return lerp(skyColor, texColor, _TextureAlpha);
    }
