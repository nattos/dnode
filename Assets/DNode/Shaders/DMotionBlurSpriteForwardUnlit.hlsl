...
)
{
    ...
    if (!forceNoMotion)
    {
		float extraMotionT = _ExtraMotion.z;
		float physicalMotionAlpha = saturate(2.0 - extraMotionT * 2.0);
		float extraMotionAlpha = saturate(extraMotionT * 2.0);
        float2 motionVec = CalculateMotionVector(inputPass.positionCS, inputPass.previousPositionCS) * physicalMotionAlpha + _ExtraMotion.xy * extraMotionAlpha;
        motionVec *= outColor.a;
