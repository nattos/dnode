...
float3 ShiftHsl(float3 color, float3 hslShift) {
    float YPrime = dot(color, kRGBToYPrime);
    float I = dot(color, kRGBToI);
    float Q = dot(color, kRGBToQ);
    float hue = abs(I) <= (1.0 / (256 * 256)) ? 0.0 : atan2(Q, I);
    float chroma = sqrt(I * I + Q * Q) * hslShift.y;
    ...
    return float3(dot(yIQ, kYIQToR), dot(yIQ, kYIQToG), dot(yIQ, kYIQToB)) * hslShift.z;

}

float3 ShiftHue(float3 color, float hueShift) {
  return ShiftHsl(color, float3(hueShift, 1, 1));
}
...
float perlinNoise(float3 pos) {
    ...
    return perlin_lerp(w, perlin_lerp(v, perlin_lerp(u, perlin_grad(p[AA], x, y, z),    // CORNER 0      
           perlin_grad(p[BA], x - 1, y, z)),                       // CORNER 1                              
           perlin_lerp(u, perlin_grad(p[AB], x, y - 1, z),                // CORNER 2                        
           perlin_grad(p[BB], x - 1, y - 1, z))),                  // CORNER 3                      
           perlin_lerp(v, perlin_lerp(u, perlin_grad(p[AA + 1], x, y, z - 1),    // CORNER 4            
           perlin_grad(p[BA + 1], x - 1, y, z - 1)),               // CORNER 5                  
           perlin_lerp(u, perlin_grad(p[AB + 1], x, y - 1, z - 1),        // CORNER 6  
           perlin_grad(p[BB + 1], x - 1, y - 1, z - 1))));         // CORNER 7
}
