    ...
    public override void DrawForeground() {
      ...
      if (BoltFlow.Configuration.showConnectionValues) {
        ...
        if (showLastValue || showPredictedvalue) {
          ...
          if (value is IDisplayDataProvider labelProvider) {
            labelShortString = labelProvider.ToShortString();
            typeForIcon = labelProvider.DisplayIconAsType;
          } else {
            labelShortString = value.ToShortString();
          }
          typeForIcon = typeForIcon ?? value?.GetType();
